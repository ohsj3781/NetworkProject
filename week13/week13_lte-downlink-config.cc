...
#include <vector>
#include "ns3/propagation-module.h"
NS_LOG_COMPONENT_DEFINE ("LteDownlinkConfig");

using namespace ns3;

int main (int argc, char *argv[])
{	
	double ltePktInv = 0.0001;
	double lteStartTimeSec = 1;
	double lteStopTimeSec = 2;
	double downlinkRb = 100; 
	double dlEarfcn = 100;
	double lteTxPower = 30;
	double distance = 10;

	CommandLine cmd;

	cmd.AddValue ("downlinkRb", "number of resource blocks in downlink",downlinkRb);
	cmd.AddValue ("ltePktInv", "packet generation interval of UDP [s]",ltePktInv);
	cmd.AddValue ("dlEarfcn", "downlink frequency",dlEarfcn);
	cmd.AddValue ("lteTxPower", "lte eNB transmission power",lteTxPower);
	...
	positionAlloc->Add (Vector(distance, 0, 0)); // for LTE UE
	...
	Config::SetDefault("ns3::LteEnbPhy::TxPower", DoubleValue (lteTxPower));
	...
	//remotesForUes - pgw
	p2pDevs = pointToPoint.Install (remoteHost, pgw);
